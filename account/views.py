...
def register(request):
    ...
    if request.method=='POST' :
        ...
        if not User.objects.filter(username=username).exists():
            if not password1 == "":
                if len(password1)>=8:
                    checkUppercase=any(char.isupper() for char in password1)
                    if checkUppercase :
                        checkDigit=any(char.isdigit() for char in password1)
                        if checkDigit :
                            ...
                            messages.error(request,"The password must contain at least one digit")
                    else:
                        messages.error(request,"The password must contain at least one uppercase letter")
                else :
                    messages.error(request,"Those passwords is not long enough (8 characters minimum)")
            else:
